...
const { broadcastAnimalCreatedNotification } = require('../services/pushNotifications');
...
router.post('/', protect, upload.array('images', 5), async (req, res) => {
  ...
  // Broadcast a push notification to all subscribers about the new listing.
  // We use `setImmediate` to ensure the response isn't delayed by the notification process, which can be slow if there are many subscribers
  // or if some endpoints are unresponsive. The notification function handles its own errors and doesn't affect the API response.
  setImmediate(() => broadcastAnimalCreatedNotification(animal, req));
