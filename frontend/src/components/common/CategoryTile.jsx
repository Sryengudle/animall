...
// Label is text-caption to fit on one line at ~110px tile width (1/3 of 388px screen).
  ...
  return (
    ...
    >
      {image ? (
        ...
        <span className="absolute inset-0 grid place-items-center text-display text-surface-400">
          {fallbackEmoji || '🐾'}
        </span>
      )}
      <span className="absolute inset-x-0 bottom-0 px-2 py-1.5 bg-gradient-to-t from-black/65 via-black/30 to-transparent">
        <span className="block text-white font-bold text-caption leading-tight drop-shadow text-center">
          {label}
        </span>
      </span>
    </button>
